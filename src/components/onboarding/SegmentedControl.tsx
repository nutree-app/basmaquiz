  ...
  return (
    ...
    >
      ...
      */}
      <span
        aria-hidden
        className="absolute inset-y-1 rounded-full bg-ob-green transition-[inset-inline-start] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
        style={{
      ...
      {options.map((option) => (
        <button
          ...
          className={`relative z-10 flex-1 rounded-full py-2 text-[14px] font-bold transition-colors duration-200 ${
            value === option.value ? "text-[#04240F]" : "text-ob-text-dim"
          }`}
        >
          {option.label}
        </button>
